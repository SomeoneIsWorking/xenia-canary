...
static void ReadLineNT(CacheLine* XE_RESTRICT destination,
  ...
  // GCC exposes this read-only intrinsic with a mutable pointer parameter.
  auto* source_xmms = const_cast<__m128i*>(source->xmms);
  __m128i first = _mm_stream_load_si128(&source_xmms[0]);
  __m128i second = _mm_stream_load_si128(&source_xmms[1]);
  __m128i third = _mm_stream_load_si128(&source_xmms[2]);
  __m128i fourth = _mm_stream_load_si128(&source_xmms[3]);
